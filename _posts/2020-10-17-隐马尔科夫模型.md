...
> P[X(t+h)&nbsp=&nbspy$\vert$X(s)&nbsp=&nbspx(t),s$\geq$t] &nbsp=&nbsp P[X(t+h)&nbsp=&nbspy$\vert$X(t)&nbsp=&nbspx(t)],&nbsph>0.
...
> P(x;$\theta$)&nbsp=&nbsp$\prod_{t=1}^{T}$P(x$_t$$\vert$x$_1$,$\cdots$,x$_{t-1}$)&nbsp=&nbspP(x$_1$)$\times$$\prod_{t=2}^{T}$P(x$_t$$\vert$x$_{t-1}$).          (公式1)
公式1表示在模型参数为$\theta$的情况下，观测状态序列x出现的概率。
