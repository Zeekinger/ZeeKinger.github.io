...
>$\prod_{t=1}^{T}$
>P(x$_t$ $\vert$ x$_1$ , $\cdots$ ,x$_{t-1}$)
>P(x$_1$)$\times$ $\prod_{t=2}^{T}$
>P(x$_t$ $\vert$ x$_{t-1}$).
>
> P(x;$\theta$)=$\prod_{t=1}^{T}$P(x$_t$$\vert$x$_1$,$\cdots$,x$_{t-1}$)=P(x$_1$)$\times$$\prod_{t=2}^{T}$P(x$_t$$\vert$x$_{t-1}$).          (公式1)
>
